...
Chaque étape affiche déjà son module (`retrieval/dense.py`, `lexical.py`, `fusion.py`, `dedup.py`, `reranker.py`) — pas besoin d'aller montrer le code séparément.
...
- `docs/schemas/ingest_fichiers.drawio` — enchaînement des fichiers de l'ingestion, options CLI, principe d'injection de dépendances (pourquoi les tests tournent sans Docker ni réseau). Pas besoin du pendant retrieval : le pipeline Streamlit affiche déjà le fichier de chaque étape.
